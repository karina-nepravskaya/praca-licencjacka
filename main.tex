...
Z tego powodu wynik PCA potraktowano wyłącznie jako wskazówkę rozpoznawczą. Pozwolił on zidentyfikować kierunek kolektywny i dwuwymiarową podprzestrzeń względną, przy czym dobrze określona jest
...
średnia $P_e$ jest zbliżona do zera, a jej 95-procentowy przedział ufności zawiera go w sobie. Nie zaobserwowano więc wyraźnego przesunięcia rozkładu w żadnym kierunku osi \(z\). Jest to zgodne z symetrią wielocyklowego impulsu laserowego, który nie wyróżnia żadnego z dwóch zwrotów osi polaryzacji. Średnie wartości \(P_e\) wraz z ich niepewnościami dla wszystkich analizowanych natężeń i kanałów jonizacji przedstawiono w tabeli~\ref{tab:Pe_srednie}.

\begin{table}[H]
\centering
\caption{Średnie wartości sumy podłużnych pędów elektronów.
Wyniki zapisano jako $\langle P_e\rangle(u)$, gdzie $u=s/\sqrt{n}$
jest standardową niepewnością średniej.}
\label{tab:Pe_srednie}
\begin{tabular}{cccc}
\toprule
$I$ [\PWcm] &
wszystkie zdarzenia [\ja] &
\emph{direct} [\ja] &
\emph{delayed} [\ja] \\
\midrule
\num{1.0} & $-0{,}08(8)$  & $-0{,}07(15)$ & $-0{,}16(10)$ \\
\num{1.3} & $-0{,}01(5)$  & $-0{,}12(9)$  & $\phantom{-}0{,}08(7)$ \\
\num{1.6} & $-0{,}03(5)$  & $-0{,}04(8)$  & $-0{,}02(6)$ \\
\num{1.7} & $\phantom{-}0{,}18(10)$ &
            $\phantom{-}0{,}32(18)$ &
            $\phantom{-}0{,}15(13)$ \\
\bottomrule
\end{tabular}
