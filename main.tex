...
W potrójnej jonizacji neonu rejestrowane lub symulowane są końcowe pędy
trzech elektronów oraz pęd jonu resztkowego. Interesującym zagadnieniem
nie jest wyłącznie rozkład pędu pojedynczego elektronu, lecz przede
wszystkim sposób, w jaki pęd zostaje podzielony między wszystkie
elektrony w obrębie jednego zdarzenia. Taka analiza powinna rozróżniać
ruch wspólny całego podukładu elektronowego od odchyleń względnych,
ponieważ te dwa rodzaje informacji mają odmienną interpretację fizyczną.
Duża suma pędów nie musi oznaczać nierównomiernego podziału, a znaczna
nierównowaga może wystąpić również przy małej sumie pędów.

Oś polaryzacji lasera jest utożsamiona z osią z. To właśnie wzdłuż tej
osi pole elektryczne nadaje elektronom największy pęd dryfu, dlatego
zasadnicza część analizy została przeprowadzona dla trzech wielkości
\ensuremath{p_{2z}}, \ensuremath{p_{3z}} i \ensuremath{p_{4z}}. Składowe poprzeczne pozostają istotne dla pełnego opisu dynamiki i zostały uwzględnione na etapie PCA. Dalszą analizę ograniczono jednak do składowych podłużnych, ponieważ to one tworzą wyróżniony w danych podobszar związany z kierunkiem polaryzacji pola.

Przyjęty schemat ma dwa cele. Po pierwsze, ma odtworzyć w sposób
czytelny informacje dostępne w reprezentacji simpleksowej dla trzech
elektronów. Po drugie, ma dostarczyć konstrukcji, którą można rozszerzać
na większą liczbę elektronów bez konieczności przedstawiania gęstości na
powierzchni coraz wyżej wymiarowego simpleksu.

\section{Dane wejściowe i konwencje}

Dla każdego zdarzenia dostępny jest dziewięciowymiarowy wektor końcowych
składowych pędu elektronów:

\begin{equation}
\mathbf{p}=\bigl(p_{2x},p_{2y},p_{2z},p_{3x},p_{3y},p_{3z},p_{4x},p_{4y},p_{4z}\bigr)^{\mathsf T}
\label{eq:wektor-pedow}
\end{equation}

W plikach analizowanych w notebooku 07\_analiza\_3D.ipynb występują
również trzy składowe pędu jonu resztkowego \ensuremath{p_1}. Pełny rekord ma zatem
dwanaście kolumn. Cząstka 1 oznacza rdzeń, elektrony 2 i 3 są w opisie
trajektoryjnym elektronami początkowo związanymi, natomiast elektron 4
jest elektronem początkowo tunelującym. W analizie podłużnej wprowadzono
skróconą notację \ensuremath{p_2} \ensuremath{\equiv} \ensuremath{p_{2z}}, \ensuremath{p_3} \ensuremath{\equiv} \ensuremath{p_{3z}} i \ensuremath{p_4} \ensuremath{\equiv} \ensuremath{p_{4z}}.

Oznaczenia elektronów są etykietami historii trajektorii, a nie
rozróżnialnymi cechami identycznych fermionów. Interpretując wielkości
zależne od numeru elektronu, trzeba więc zachować ostrożność. W
szczególności różnica \ensuremath{p_2}\ensuremath{-}\ensuremath{p_3} opisuje asymetrię pomiędzy dwiema
trajektoriami oznaczonymi jako początkowo związane, natomiast nie
definiuje obserwabli przypisanej do dwóch fizycznie odróżnialnych
elektronów. Z tego powodu szczególnie wartościowe są również miary
niezmiennicze na permutacje, takie jak suma pędów, moduł nierównowagi K,
\Neff{}, \xmax{} oraz entropia podziału.

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.3333}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.3333}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.3333}}@{}}
\caption{Konwencja etykiet stosowana w plikach danych i notebookach.}\label{tab:konwencja-etykiet}\\
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
\textbf{Obiekt}
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
\textbf{Składowe użyte w analizie}
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
\textbf{Znaczenie robocze}
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
jon resztkowy (1) & \ensuremath{p_{1z}} & kontrola bilansu pędu wzdłuż osi z \\
elektron 2 & \ensuremath{p_{2z}} & elektron początkowo związany \\
elektron 3 & \ensuremath{p_{3z}} & elektron początkowo związany \\
elektron 4 & \ensuremath{p_{4z}} & elektron początkowo tunelujący \\
\end{longtable}

\section{Reprezentacja simpleksowa i jej ograniczenia}

W reprezentacji simpleksowej rozpatruje się udziały bezwzględnego pędu
podłużnego poszczególnych elektronów. Dla trzech elektronów można je
zdefiniować jako

\begin{equation}
x_i=\frac{|p_i|}{|p_2|+|p_3|+|p_4|},\qquad i\in\{2,3,4\}
\label{eq:udzialy-pedu}
\end{equation}

\begin{equation}
x_2+x_3+x_4=1,\qquad x_i\ge 0
\label{eq:normalizacja-udzialow}
\end{equation}

Warunek normalizacji sprawia, że każde zdarzenie odpowiada punktowi w
trójkącie. Położenie punktu koduje względny podział wartości
bezwzględnych, natomiast znaki pędów trzeba przechowywać oddzielnie. Dla
trzech elektronów jest to reprezentacja wyjątkowo wygodna: cała
informacja o znormalizowanych udziałach mieści się na płaszczyźnie.

Trudność pojawia się przy zwiększaniu liczby elektronów. Dla czterech
elektronów udziały tworzą tetraedr, a dla N elektronów --- simpleks o
wymiarze \ensuremath{N-1}. Już w przypadku tetraedru przedstawianie gęstości wewnątrz
bryły lub na jej powierzchni jest znacznie mniej przejrzyste niż
dwuwymiarowa mapa w trójkącie. W wyższych wymiarach nie istnieje
pojedyncza wizualizacja, która zachowuje pełną informację i pozostaje
równie czytelna.

Zaproponowana metoda nie odrzuca idei simpleksu. Rozdziela ją na kilka
obserwabli o jasnej interpretacji: miary równomierności podziału,
największy udział, entropię, sektory znaków oraz współrzędne
logarytmicznych ilorazów. Dzięki temu poszczególne własności zdarzeń
można analizować na osobnych wykresach, zamiast próbować zakodować
wszystkie naraz w geometrii wielowymiarowej bryły.

\chapter{Analiza głównych składowych}

\section{Przygotowanie danych do PCA}

W notebooku 05\_PCA.ipynb PCA została przeprowadzona w przestrzeni
dziewięciu składowych pędu elektronów. Dane zostały zestawione w
macierzy o kolumnach \ensuremath{p_{2x}}, \ensuremath{p_{2y}}, \ensuremath{p_{2z}}, \ensuremath{p_{3x}}, \ensuremath{p_{3y}}, \ensuremath{p_{3z}}, \ensuremath{p_{4x}}, \ensuremath{p_{4y}} i \ensuremath{p_{4z}}, a
następnie wystandaryzowane za pomocą transformacji StandardScaler.
Standaryzacja usuwa różnice skali pomiędzy kolumnami i powoduje, że PCA
jest wykonywana na strukturze korelacyjnej, a nie wyłącznie na
bezwzględnych wariancjach poszczególnych składowych.
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.2000}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.2000}}@{}}
\caption{Współczynniki składowych z w trzech kierunkach PCA zdominowanych przez ruch podłużny. Znak wektora własnego PCA jest umowny.}\label{tab:wspolczynniki-pca}\\
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
\textbf{Kanał}
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\chapter{Współrzędne kolektywne i względne}
...
\label{eq:wspolrzedna-q}
\end{equation}

\begin{equation}
\xi_1=\frac{p_2-p_3}{\sqrt2}
\label{eq:wspolrzedna-xi1}
\end{equation}

\begin{equation}
\xi_2=\frac{p_2+p_3-2p_4}{\sqrt6}
\label{eq:wspolrzedna-xi2}
\end{equation}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.2000}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.2000}}@{}}
\caption{Macierz ortonormalnej transformacji Jacobiego--Helmerta.}\label{tab:macierz-helmerta}\\
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
\textbf{Współrzędna}
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\label{eq:wektor-pedow1}
\end{equation}

\begin{equation}
\bar p=\frac{p_2+p_3+p_4}{3}
\label{eq:wektor-pedow2}
\end{equation}

Z równania~\eqref{eq:wektor-pedow1} wynika, że K jest niezmiennicze na dowolną permutację
...
\begin{equation}
A=|p_2|+|p_3|+|p_4|
\label{eq:wektor-pedow7}
\end{equation}

oraz udziały \ensuremath{x_2}, \ensuremath{x_3} i \ensuremath{x_4} zgodnie z równaniem~\eqref{eq:udzialy-pedu}. Na ich podstawie
...
\chapter{Przebieg analizy i kontrola poprawności}
...
Dodatkowo wykonano rekonstrukcję pędów na podstawie równań~\eqref{eq:wektor-pedow4}--\eqref{eq:wektor-pedow6}.
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\columnwidth - 2\tabcolsep) * \real{0.5000}}@{}}
\caption{Numeryczne testy ortogonalności i odwracalności transformacji.}\label{tab:test-transformacji}\\
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
\textbf{Test}
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\caption{Gęstość zdarzeń na płaszczyźnie względnej $(\xi_1,\xi_2)$ dla natężenia $I=1{,}7\,\mathrm{PW/cm^2}$: kanał \delayed{} po lewej i \direct{} po prawej. W obu panelach zastosowano tę samą bazę Jacobiego--Helmerta.}
...
\caption{Zależność pomiędzy całkowitym podłużnym pędem elektronów $P_e$ a modułem nierównowagi względnej $K$ dla kanałów \direct{} i \delayed{} przy $I=1{,}7\,\mathrm{PW/cm^2}$.}
\label{fig:p-vs-k}
\end{figure}
...
\caption{Rozkład efektywnej liczby elektronów uczestniczących w podziale bezwzględnego pędu dla kanałów \direct{} i \delayed{} przy $I=1{,}7\,\mathrm{PW/cm^2}$.}
\label{fig:neff}
\end{figure}
...
mieszane mają niewielką liczebność. Wskazuje to na znaczny udział zdarzeń, w których
...
\caption{Liczebności ośmiu sektorów znakowych $(p_{2z},p_{3z},p_{4z})$ dla kanałów \direct{} i \delayed{} przy $I=1{,}7\,\mathrm{PW/cm^2}$.}
...
\caption{Gęstość w przestrzeni ilr znormalizowanych udziałów $|p_i|$ dla kanałów \direct{} i \delayed{} przy $I=1{,}7\,\mathrm{PW/cm^2}$.}
\label{fig:ilr}
\end{figure}
...
\caption{Macierze korelacji Pearsona pomiędzy podłużnymi pędami elektronów dla kanałów \direct{} i \delayed{} przy $I=1{,}7\,\mathrm{PW/cm^2}$.}
...
zdarzeń delayed. Tabele~\ref{tab:porownanie-pedy} i~\ref{tab:porownanie-udzialy} zestawiają główne wartości liczbowe.
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\columnwidth - 16\tabcolsep) * \real{0.1111}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 16\tabcolsep) * \real{0.1111}}@{}}
\caption{Wielkości kolektywne, względne i korelacje parami.}\label{tab:porownanie-pedy}\\
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
\textbf{Kanał}
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\columnwidth - 14\tabcolsep) * \real{0.1250}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 14\tabcolsep) * \real{0.1250}}@{}}
\caption{Miary równomierności podziału i dominujące sektory znaków.}\label{tab:porownanie-udzialy}\\
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
\textbf{Kanał}
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\begin{itemize}
\item
  Dla $N>3$ pełna przestrzeń względna ma wymiar $N-1$. Sam skalar
  ...
  projekcji, a nie redukcji pełnego opisu do dwóch osi.
\item
...
Wyniki analizy głównych składowych wskazały, że podłużną dynamikę trzech
elektronów można opisać za pomocą jednego trybu
...
Moduł K stanowi prostą, permutacyjnie niezmienniczą miarę nierównowagi.
