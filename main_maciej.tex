...
\label{eq:pc1}
\end{equation}
Pozostałe lecz współczynniki PC1 są zaniedbywalnie małe. Oznacza to, że ten kierunek jest zbliżony do wektora $(1,1,1)$ w przestrzeni $(p_2,p_3,p_4)$, a więc opisuje wspólne przesunięcie wszystkich trzech pędów.

Dwa inne kierunki, PC8 i PC9, również są zdominowane przez składowe $z$, ale ich współczynniki mają przeciwne znaki. Dla klasy zdarzeń \direct{} otrzymano wartości przedstawione w tabeli~\ref{tab:pca-z}. Dla \delayed{} również pojawia się jeden kierunek bliski $(1,1,1)$. Z kolei dwa pozostałe kierunki o niezerowych współczynnikach jedynie przy osi polaryzacji różnią się od tych otrzymanych podczas \direct{}, chociaż dalej one opisują różnice pomiędzy
pędami elektronów.
...
PCA wyznacza więc dla obu klas zdarzeń tę samą dwuwymiarową przestrzeń ruchu względnego, jednak jej osie zależą od analizowanego zbioru danych. Dla innej klasy zdarzeń (\direct{} lub \delayed{}) lub innego natężenia pola współczynniki przy \(p_2\), \(p_3\) i \(p_4\) mogą się zmienić, przez co PC8 i PC9 nie muszą mieć tego samego znaczenia w kolejnych analizach. Z tego powodu zamiast używać bezpośrednio osi PCA, wygodniej jest wprowadzić jedną, stałą bazę, wspólną dla wszystkich porównywanych zbiorów.
