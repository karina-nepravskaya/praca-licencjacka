...
Spośród pozostałych składowych PCA również PC8 i PC9 są zdominowane przez składowe wzdłuż osi $z$. W przeciwieństwie do PC1 ich współczynniki przy $p_{2z}$, $p_{3z}$ i $p_{4z}$ mają różne znaki, dlatego kierunki te opisują przede wszystkim zmiany względne pomiędzy pędami elektronów. Analogiczną strukturę otrzymuje się dla klasy zdarzeń \delayed{}.

Orientacja osi PC8 i PC9 zależy od analizowanego zbioru danych. Przy zmianie klasy zdarzeń lub natężenia pola mogą one zmienić orientację w obrębie podprzestrzeni ruchu względnego. Z tego powodu w dalszej analizie nie wykorzystujemy bezpośrednio PC8 i PC9 jako współrzędnych, lecz definiujemy stałą bazę tej podprzestrzeni.

\section{Współrzędne kolektywne i względne}

W przestrzeni podłużnych pędów $(p_2,p_3,p_4)$ kierunek kolektywny wskazany przez PC1 odpowiada w przybliżeniu wektorowi $(1,1,1)$. Zgodnie z interpretacją PC8 i PC9 jako dwóch niezależnych kierunków względnych pozostałe dwie osie wybieramy w płaszczyźnie prostopadłej do tego kierunku. Dla wektora $\mathbf v=(v_2,v_3,v_4)$ płaszczyzna ta jest określona warunkiem
\begin{equation}
v_2+v_3+v_4=0.
\label{eq:relative-plane}
\end{equation}

Jako pierwszy kierunek w tej płaszczyźnie przyjmujemy różnicę pomiędzy pędami elektronów 2 i 3, której odpowiada wektor $(1,-1,0)$. Drugi kierunek wyznaczamy z warunku ortogonalności zarówno względem $(1,1,1)$, jak i $(1,-1,0)$. Otrzymujemy wówczas wektor proporcjonalny do $(1,1,-2)$. Po normalizacji trzy kierunki mają postać
\begin{equation}
...
\end{equation}
Wektor $\mathbf e_0$ stanowi stałą, znormalizowaną reprezentację kierunku kolektywnego wskazanego przez PC1. Wektory $\mathbf e_1$ i $\mathbf e_2$ tworzą ortonormalną bazę płaszczyzny ruchu względnego wskazanej przez PC8 i PC9. Wektor $\mathbf e_1$ opisuje różnicę pędów elektronów 2 i 3, natomiast $\mathbf e_2$ porównuje ich wspólny ruch z ruchem elektronu 4.
