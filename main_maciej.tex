...
W badaniach wielokrotnej jonizacji istotna jest nie tylko energia końcowa emitowanych elektronów, lecz również sposób, w jaki elektrony dzielą pomiędzy siebie pęd. W przypadku potrójnej jonizacji, po półklasycznym przygotowaniu warunków
początkowych i dalszej klasycznej propagacji układu~\cite{Emmanouilidou2023}, otrzymujemy dla
każdego zdarzenia końcowe pędy wszystkich czterech cząstek (jonu oraz
trzech elektronów). W wyniku obliczeń klasycznych znamy dodatkowo ich wszystkie składowe. W dalszej analizie będziemy przyglądać się jedynie elektronom, więc pełny punkt danych zapisujemy jako
\begin{equation}
\mathbf p=
...
Analizowane dane zostały otrzymane dla czterech natężeń pola laserowego:
\(1{,}0\), \(1{,}3\), \(1{,}6\) oraz \(1{,}7\,\mathrm{PW/cm^2}\).
Dla każdego natężenia wszystkie zdarzenia potrójnej jonizacji są
dzielone na dwie klasy: zdarzenia bezpośrednie i opóźnione, dalej
oznaczane odpowiednio jako \direct{} i \delayed{}. Dostępny jest również
zbiór obejmujący wszystkie zdarzenia (\textit{all events}).
...
Punktem wyjścia była Analiza Głównych Składowych (z ang. Principal Component Analysis), dalej PCA. Została ona wykonana w pełnej dziewięciowymiarowej przestrzeni z równania~\eqref{eq:data-vector}. PCA pokazała, że wśród dziewięciu kierunków wyróżnia się trójwymiarowy podobszar związany prawie wyłącznie ze składowymi wzdłuż $z$. Jeden z tych kierunków odpowiada wspólnemu ruchowi wszystkich elektronów, a dwa pozostałe opisują ich wzajemne różnice. To spostrzeżenie doprowadziło do wprowadzenia stałej bazy kolektywnej i względnej, niezależnej od konkretnego zbioru danych.

\section{Analiza Głównych Składowych}

Przed wykonaniem analizy każda z dziewięciu kolumn została wystandaryzowana, tak aby miała zerową średnią i jednostkowe odchylenie standardowe. Dzięki temu wynik nie jest zdominowany przez samą różnicę skali poszczególnych składowych pędu.

Analiza PCA wyznacza nowe, wzajemnie ortogonalne kierunki w przestrzeni dziewięciu składowych pędu elektronów. Pierwszy z tych kierunków, oznaczany jako PC1, odpowiada największej części zmienności danych. Dla zbioru danych odpowiadającego natężeniu pola
\(I=1{,}7\,\mathrm{PW/cm^2}\) i klasie zdarzeń \direct{}
kierunek ten wyjaśnia około \(29{,}7\%\) całkowitej wariancji danych. Współczynniki przy składowych przyosiowych $p_{2z}$, $p_{3z}$ i $p_{4z}$ dla tego kierunku są niemal jednakowe,
\begin{equation}
\mathrm{PC1}\approx
0{,}580\,p_{2z}+0{,}580\,p_{3z}+0{,}568\,p_{4z}.
\label{eq:pc1}
\end{equation}
Pozostałe współczynniki PC1 są zaniedbywalnie małe. Oznacza to, że ten kierunek jest zbliżony do wektora $(1,1,1)$ w przestrzeni $(p_2,p_3,p_4)$, a więc opisuje wspólne przesunięcie wszystkich trzech pędów.

Spośród pozostałych składowych PCA również PC8 i PC9 są zdominowane przez składowe wzdłuż osi $z$. W przeciwieństwie do PC1 ich współczynniki przy $p_{2z}$, $p_{3z}$ i $p_{4z}$ mają różne znaki, dlatego kierunki te opisują przede wszystkim zmiany względne pomiędzy pędami elektronów. Dla danych przy \(I=1{,}7\,\mathrm{PW/cm^2}\) współczynniki tych trzech składowych zestawiono w tabeli~\ref{tab:pca-z}.

\begin{table}[htbp]
\centering
\caption{Współczynniki przy $p_{2z}$, $p_{3z}$ i $p_{4z}$ dla składowych PCA zdominowanych przez ruch wzdłuż osi polaryzacji przy $I=1{,}7\,\mathrm{PW/cm^2}$. Znak wektora własnego PCA jest umowny.}
\label{tab:pca-z}
\begin{tabular}{llrrr}
\toprule
klasa zdarzeń & składowa & $p_{2z}$ & $p_{3z}$ & $p_{4z}$\\
\midrule
\direct  & PC1 &  0,580 &  0,580 &  0,568\\
\direct  & PC8 & -0,402 & -0,402 &  0,822\\
\direct  & PC9 & -0,707 &  0,707 & -0,001\\
\delayed & PC1 &  0,545 &  0,533 &  0,645\\
\delayed & PC8 & -0,644 &  0,695 & -0,031\\
\delayed & PC9 & -0,480 & -0,429 &  0,761\\
\bottomrule
\end{tabular}
\end{table}

W obu klasach PC1 ma ten sam jakościowy charakter: wszystkie trzy współczynniki podłużne mają ten sam znak i podobne wartości. Natomiast osie PC8 i PC9 mają inną orientację dla \direct{} i \delayed{}. Pokazuje to, że PCA wskazuje tę samą ideę rozdzielenia ruchu kolektywnego od względnego, ale nie dostarcza jednej stałej pary osi względnych, którą można bezpośrednio porównywać między zbiorami. Z tego powodu w dalszej analizie wprowadzamy własną, niezmienną bazę tej przestrzeni.

\section{Współrzędne kolektywne i względne}

Na podstawie wyniku PCA można stwierdzić, że przestrzeń trzech pędów
podłużnych można rozdzielić na jeden kierunek kolektywny oraz
dwuwymiarową podprzestrzeń ruchu względnego. W dalszej analizie nie
wykorzystujemy jednak bezpośrednio wartości współczynników PC1, PC8 i PC9, ponieważ zależą one nieco od konkretnego zbioru danych na której przeprowadzamy analizę. W tym ujęciu PCA stanowi jedynie punkt wyjścia, wskazując ogólną strukturę nowej bazy, a nie definiując bezpośrednio jej osi.

Kierunek kolektywny powinien traktować trzy elektrony jednakowo, dlatego przyjmujemy kierunek $(1,1,1)$, zgodny z postacią PC1 otrzymaną numerycznie. Wektory opisujące wyłącznie ruch względny muszą być do niego prostopadłe. Dla wektora $\mathbf v=(v_2,v_3,v_4)$ oznacza to warunek
\begin{equation}
v_2+v_3+v_4=0.
\label{eq:relative-plane}
\end{equation}
Warunek ten definiuje dwuwymiarową płaszczyznę, więc potrzebujemy w niej dwóch niezależnych osi.

Pierwszą oś wybieramy jako różnicę pędów elektronów 2 i 3, czemu
odpowiada kierunek $(1,-1,0)$. Taki charakter ma przede wszystkim PC9
dla klasy zdarzeń \direct{} oraz PC8 dla klasy \delayed{}, dla których
współczynniki przy $p_{2z}$ i $p_{3z}$ mają przeciwne znaki, a udział
$p_{4z}$ jest niewielki.

Drugą oś wybieramy w kierunku opisującym wspólną zmianę $p_{2z}$ i
$p_{3z}$ względem $p_{4z}$. Strukturę tę wskazuje przede wszystkim PC8
dla klasy \direct{} oraz PC9 dla klasy \delayed{}. Po ustaleniu pierwszej
osi drugi kierunek jest wyznaczony, z dokładnością do znaku, przez
warunek ortogonalności zarówno do kierunku kolektywnego $(1,1,1)$,
jak i do $(1,-1,0)$. Otrzymujemy wówczas kierunek $(1,1,-2)$.

Po normalizacji trzy wektory mają postać
\begin{equation}
...
\end{equation}
Wybór $\mathbf e_1$ ustala orientację osi wewnątrz płaszczyzny względnej w prosty do interpretacji sposób: $\mathbf e_1$ porównuje elektrony 2 i 3, natomiast $\mathbf e_2$ porównuje ich wspólny ruch z ruchem elektronu 4. Inna para ortonormalnych osi mogłaby rozpiąć tę samą płaszczyznę, ale powyższa baza jest stała dla wszystkich analizowanych zbiorów.

Rzutując wektor $(p_2,p_3,p_4)$ na tę bazę, otrzymujemy
\begin{equation}
...
\label{eq:Pe}
\end{equation}
Natomiast $\xi_1$ opisuje różnicę pędów elektronów 2 i 3, a $\xi_2$ porównuje średni ruch tej pary z elektronem 4. W ten sposób para $(\xi_1,\xi_2)$ zawiera całą informację o ruchu względnym wzdłuż osi $z$.
...
\begin{itemize}
    ...
    \item na końcu porównujemy te same wielkości dla klas zdarzeń \direct{} i \delayed{} przy tym samym natężeniu pola.
\end{itemize}
...
Zacznijmy od porównania $P_e$ i $K$. Średnia wartość $P_e$ jest niewielka w obu klasach zdarzeń, co jest zgodne z występowaniem zdarzeń o obu zwrotach pędu wzdłuż osi polaryzacji. Bardziej użyteczna jest szerokość rozkładu. Odchylenie standardowe $P_e$ wynosi około $7{,}54$ dla klasy zdarzeń \direct{} oraz $5{,}38$ dla \delayed{}. Jednocześnie średnie $K$ wynosi odpowiednio $1{,}40$ i $2{,}38$.
...
\toprule
klasa zdarzeń & $n$ & $\sigma(P_e)$ & $\langle K\rangle$ & $\langle N_{\mathrm{eff}}\rangle$ & $\langle x_{\max}\rangle$ & $\langle H\rangle$\\
\midrule
...
Mniejsze $K$ dla \direct{} oznacza, że trzy pędy częściej leżą blisko kierunku kolektywnego $(1,1,1)$. Jednocześnie większa szerokość rozkładu $P_e$ pokazuje, że zgodny ruch wszystkich elektronów może osiągać większe wartości bez konieczności dużego rozrzutu pomiędzy nimi. W klasie zdarzeń \delayed{} sytuacja jest odwrotna: całkowity pęd ma węższy rozkład, ale różnice wewnątrz trójki są większe.

\section{Równomierność podziału pędu}

Ten sam wniosek można sprawdzić niezależnie za pomocą udziałów $x_i$. Dla \direct{} otrzymujemy $\langle N_{\mathrm{eff}}\rangle=2{,}628$, podczas gdy dla \delayed{} średnia spada do $2{,}374$. Jednocześnie średni największy udział rośnie z $0{,}473$ do $0{,}533$, a średnia entropia maleje z $0{,}919$ do $0{,}846$. Wszystkie trzy miary zmieniają się więc w tym samym kierunku. W klasie zdarzeń \direct{} wartość bezwzględna pędu jest przeciętnie dzielona bardziej równomiernie pomiędzy trzy elektrony, natomiast w \delayed{} częściej pojawia się elektron o wyraźnie większym udziale.
...
Dopełnieniem analizy wartości bezwzględnych są sektory znakowe. W obu klasach zdarzeń najczęstszy jest sektor $+++$, jednak jego udział wyraźnie się różni. Dla \direct{} wynosi około $47{,}4\%$, a dla \delayed{} około $23{,}0\%$. Oznacza to, że w klasie zdarzeń \direct{} znacznie częściej wszystkie trzy elektrony mają końcowy pęd skierowany w tę samą stronę osi $z$. Sama reprezentacja oparta na $|p_i|$ tej informacji nie zawiera.

Podobną różnicę widać we współczynnikach korelacji Pearsona. Zebrano je w tabeli~\ref{tab:corr}. W klasie zdarzeń \direct{} wszystkie trzy korelacje są silnie dodatnie. W \delayed{} korelacja pomiędzy elektronami 2 i 3 jest znacznie słabsza, a pozostałe dwie mają wartości umiarkowane.
...
\begin{tabular}{lrrr}
\toprule
klasa zdarzeń & $r(p_2,p_3)$ & $r(p_2,p_4)$ & $r(p_3,p_4)$\\
\midrule
...
Dla $I=1{,}7\,\mathrm{PW/cm^2}$ klasy zdarzeń \direct{} i \delayed{} różnią się konsekwentnie we wszystkich głównych obserwablach. Klasa zdarzeń \direct{} ma mniejsze $K$, większe $N_{\mathrm{eff}}$, mniejsze $x_{\max}$ i silniejsze dodatnie korelacje pomiędzy pędami elektronów. Klasa zdarzeń \delayed{} charakteryzuje się większą nierównowagą i częstszymi konfiguracjami o różnych znakach pędów. Wyniki są więc zgodne z bardziej kolektywnym charakterem zdarzeń \direct{} i bardziej rozdzieloną dynamiką zdarzeń \delayed{}.
