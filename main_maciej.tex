...
W obu klasach PC1 ma ten sam jakościowy charakter: wszystkie trzy współczynniki podłużne mają ten sam znak i podobne wartości. Natomiast osie PC8 i PC9 mają inną orientację dla \direct{} i \delayed{}. Pokazuje to, że PCA wskazuje tę samą ideę rozdzielenia ruchu kolektywnego od względnego, ale nie dostarcza bespośrednio jednej stałej pary osi względnych dla obu analizowanych zbiorów. Z tego powodu w dalszej analizie musimy samodzielnie wprowadzić tą stałą bazę, korzystając z uzyskanych metodą PCA współczynników ortogonalnych do siebie kierunków.

\section{Współrzędne kolektywne i względne}

Początkowo przedstawiamy pojedyncze zdarzenie jako wektor trzech podłużnych składowych pędu elektronów
\begin{equation}
\mathbf{p}_z=
\bigl(p_2,p_3,p_4\bigr)^{\mathsf T}.
\label{eq:5}
\end{equation}

W obu klasach zdarzeń PC1 ma współczynniki przy $p_{2z}$, $p_{3z}$ i
$p_{4z}$ o tym samym znaku i zbliżonych wartościach. Kierunek
kolektywny przyjmujemy zatem jako $(1,1,1)$.

Wektory opisujące wyłącznie ruch względny muszą być prostopadłe do zdefiniowanego kierunku kolektywnego. Dla wektora $\mathbf v=(v_2,v_3,v_4)$ oznacza to warunek
\begin{equation}
v_2+v_3+v_4=0.
